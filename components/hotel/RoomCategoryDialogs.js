...
export function EditRoomCategoryDialog({ children, roomCategory, onRoomCategoryUpdated }) {
    const [open, setOpen] = useState(false);
    ...
    const handleSubmit = async (e) => {
        ...
        if (response.ok) {
            ...
            const data = await response.json();
            setError(data.message || "Ocorreu um erro ao editar a categoria de quarto.");
            if (data.action) {
                setAction(data.action);
            }
            setIsErrorDialogOpen(true);
