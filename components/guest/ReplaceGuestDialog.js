...
export function ReplaceGuestDialog({ children, sale, oldGuest, onSuccess }) {
  ...
  const handleConfirmSwap = async () => {
    ...
    try {
      const res = await fetch(`/api/v1/sales/${sale.id}/replace-guest`, {
        method: "POST",
        headers: {
      ...
      if (res.ok) {
        ...
        const data = await res.json()
        setError(data.message || "Erro ao realizar a troca de hóspede.")
        setAction(
          data.action || "Verifique os dados informados ou tente novamente.",
        )
  ...
  return (
    <>
      ...
      <ErrorDialog
        ...
        title="Erro ao Substituir Hóspede"
        message={error}
