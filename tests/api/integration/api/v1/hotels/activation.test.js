...
describe("PATCH /api/v1/hotels/[id] (Activation)", () => {
  test("activating one hotel should deactivate others", async () => {
    ...
    // Create Hotel A
    const hotelAResponse = await fetch(`${orchestrator.webserverUrl}/api/v1/hotels`, {
      method: "POST",
      headers: {
    ...
    // Create Hotel B
    const hotelBResponse = await fetch(`${orchestrator.webserverUrl}/api/v1/hotels`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Cookie: `session_id=${session.token}`,
    ...
    // 2. Activate Hotel A
    const activateAResponse = await fetch(`${orchestrator.webserverUrl}/api/v1/hotels/${hotelA.id}`, {
      method: "PATCH",
      headers: {
        "Content-Type": "application/json",
        Cookie: `session_id=${session.token}`,
    ...
    const activateBResponse = await fetch(`${orchestrator.webserverUrl}/api/v1/hotels/${hotelB.id}`, {
      method: "PATCH",
      headers: {
        "Content-Type": "application/json",
        Cookie: `session_id=${session.token}`,
    ...
    const getAResponse = await fetch(`${orchestrator.webserverUrl}/api/v1/hotels/${hotelA.id}`, {
      headers: {
        Cookie: `session_id=${session.token}`,
      },
    })
