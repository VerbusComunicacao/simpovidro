...
describe("GET /api/v1/hotels/active", () => {
  describe("Anonymous user", () => {
    test("Can list active hotels with details", async () => {
      ...
      const roomType = await orchestrator.createRoomType(adminUser.id, { name: "Suite" })
      const roomCategory = await orchestrator.createRoomCategory(adminUser.id, { name: "Deluxe" })
