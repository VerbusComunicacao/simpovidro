...
describe("Shared Room Availability (Parent-Child Inheritance)", () => {
  test("Child room should inherit values from parent, sync availability, and prevent invalid overrides", async () => {
    ...
    const categoryParent = await orchestrator.createRoomCategory(createdUser.id, { name: "Luxo Single" })
    const categoryChild = await orchestrator.createRoomCategory(createdUser.id, { name: "Luxo Duplo" })
    ...
    const parentResponse = await fetch(`http://localhost:3000/api/v1/rooms/${parentRoom.id}`, {
      headers: { Cookie: `session_id=${sessionObject.token}` },
    }).then(r => r.json())

    const childResponse = await fetch(`http://localhost:3000/api/v1/rooms/${childRoom.id}`, {
      headers: { Cookie: `session_id=${sessionObject.token}` },
    }).then(r => r.json())
    ...
    const updateChildResponse = await fetch(`http://localhost:3000/api/v1/rooms/${childRoom.id}`, {
      method: "PATCH",
    ...
    expect(errBody.message).toContain("Este quarto é um quarto filho e herda a disponibilidade do quarto pai")

    // 6. Update total_rooms on parent room
    const updateParentResponse = await fetch(`http://localhost:3000/api/v1/rooms/${parentRoom.id}`, {
      method: "PATCH",
      body: JSON.stringify({ total_rooms: 8 }),
      headers: {
        "Content-Type": "application/json",
    ...
    const parentResponse2 = await fetch(`http://localhost:3000/api/v1/rooms/${parentRoom.id}`, {
      headers: { Cookie: `session_id=${sessionObject.token}` },
    }).then(r => r.json())

    const childResponse2 = await fetch(`http://localhost:3000/api/v1/rooms/${childRoom.id}`, {
      headers: { Cookie: `session_id=${sessionObject.token}` },
    }).then(r => r.json())
    ...
    const parentResponse3 = await fetch(`http://localhost:3000/api/v1/rooms/${parentRoom.id}`, {
      headers: { Cookie: `session_id=${sessionObject.token}` },
    }).then(r => r.json())

    const childResponse3 = await fetch(`http://localhost:3000/api/v1/rooms/${childRoom.id}`, {
      headers: { Cookie: `session_id=${sessionObject.token}` },
    }).then(r => r.json())
