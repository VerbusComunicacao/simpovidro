...
describe("POST /api/v1/registrations", () => {
  ...
  describe("Authenticated User", () => {
    ...
    test("Scenario: Register same 2 people twice with updated details (Upsert Verification)", async () => {
      ...
      const hotel2 = await orchestrator.createHotel(hotelOwner.id, { name: "Other Hotel" })
      const room2 = await orchestrator.createRoom(hotelOwner.id, { hotel_id: hotel2.id, available_rooms: 5 })
    ...
    test("Error: Should fail if only one child is added to a single room (max_adults: 1, max_children: 0)", async () => {
        const singleCategory = await orchestrator.createRoomCategory(hotelOwner.id, {
            name: "Single Room",
            max_adults: 1,
            max_children: 0
        })
    ...
    test("Scenario: 2 Adults / 2 Children capacity validation", async () => {
        const familyCategory = await orchestrator.createRoomCategory(hotelOwner.id, {
            name: "Family Room",
            max_adults: 2,
            max_children: 2
        })
        ...
        const res4Children = await fetch(`${orchestrator.webserverUrl}/api/v1/registrations`, {
            method: "POST",
            headers: { "Content-Type": "application/json", Cookie: `session_id=${session.token}` },
            body: JSON.stringify({
        ...
        expect((await res4Children.json()).message).toContain("número de crianças (4) excede a capacidade máxima")

        // 2. Try 3 adults + 1 child -> Fail (max adults is 2)
        const res3Adults = await fetch(`${orchestrator.webserverUrl}/api/v1/registrations`, {
            method: "POST",
            headers: { "Content-Type": "application/json", Cookie: `session_id=${session.token}` },
            body: JSON.stringify({
                room_id: familyRoom.id,
        ...
        expect((await res3Adults.json()).message).toContain("número de adultos (3) excede a capacidade máxima")

        // 3. Try 2 adults + 2 children -> Success
        const resSuccess = await fetch(`${orchestrator.webserverUrl}/api/v1/registrations`, {
            method: "POST",
            headers: { "Content-Type": "application/json", Cookie: `session_id=${session.token}` },
            body: JSON.stringify({
                room_id: familyRoom.id,
    ...
    test("Scenario: Price policy 'free under 12' (max_age: 12) validation", async () => {
        ...
        const testCategory = await orchestrator.createRoomCategory(hotelOwner.id, {
        ...
        expect(Number(saleResult.rows[0].total_amount)).toBe(400.00)
    })
    ...
    test("Scenario: Registering same guest in the same hotel/period twice should fail", async () => {
        ...
        const response1 = await fetch(`${orchestrator.webserverUrl}/api/v1/registrations`, {
            method: "POST",
            headers: { "Content-Type": "application/json", Cookie: `session_id=${session.token}` },
            body: JSON.stringify({
                room_id: room.id,
        ...
        const response2 = await fetch(`${orchestrator.webserverUrl}/api/v1/registrations`, {
            method: "POST",
            headers: { "Content-Type": "application/json", Cookie: `session_id=${session.token}` },
            body: JSON.stringify({
                room_id: room.id,
