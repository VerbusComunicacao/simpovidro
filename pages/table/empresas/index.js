...
import { Plus, Search, Loader2, MapPin, Phone, Mail, User, Pencil } from "lucide-react";
    ...
    <TableLayout>
      <div className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500">Listagem de todas as empresas cadastradas no sistema.</p>
        </div>
      ...
      {filteredCompanies && filteredCompanies.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredCompanies.map((company) => (
            <Card key={company.id} className="overflow-hidden border-none shadow-lg hover:shadow-xl transition-all duration-300">
              <div className="bg-blue-600 p-6 text-white relative">
                <div className="flex justify-between items-start gap-4">
                  ...
                  <div className="flex flex-col items-end gap-2">
                    ...
                    <CompanyDialog onCompanySuccess={mutate} companyToEdit={company}>
              ...
              <CardContent className="p-6 space-y-6">
                <div className="flex items-start gap-3">
                  ...
                  <div className="space-y-0.5">
                    ...
                    {company.address_complement && (
                      <p className="text-xs text-gray-500 italic">{company.address_complement}</p>
                    )}
                    <p className="text-xs text-gray-400">{company.neighborhood}</p>
                    <p className="text-sm font-bold text-gray-900">{company.city} - {company.state}</p>
                  </div>
                </div>
                ...
                <div className="flex justify-between items-center pt-2">
                  ...
                  {company.discount_status === 'S' && (
                    <Badge variant="outline" className="border-gray-200 text-gray-700 bg-gray-50 px-4 py-1">
                      Desconto
                    </Badge>
                  )}
                </div>
      ...
      <div className="fixed bottom-8 right-8 z-50">
        <CompanyDialog onCompanySuccess={mutate}>
          <Button size="lg" className="h-14 w-14 rounded-full shadow-2xl hover:scale-110 transition-transform p-0">
