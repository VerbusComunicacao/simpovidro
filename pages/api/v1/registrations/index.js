...
async function postHandler(request, response) {
  ...
  for (const currentGuestData of guests_data) {
    if (currentGuestData.cpf_number && usedCPFs.has(currentGuestData.cpf_number)) {
      throw new ValidationError({
  ...
  try {
    ...
    const formatGuestDetails = (guest) => {
      const fields = [
        ...
        { label: "Nascimento", value: guest.birth_date ? formatDate(guest.birth_date) : null },
        { label: "Email", value: guest.email },
        { label: "Celular", value: guest.phone },
        { label: "Endereço", value: guest.address ? `${guest.address}, ${guest.address_number || ''} ${guest.address_complement ? `(${guest.address_complement})` : ''} - ${guest.city}/${guest.state}` : null },
        { label: "Medicamentos", value: guest.medication_details },
        { label: "Obs. Saúde", value: guest.health_observations },
      ].filter(f => f.value)

      return fields.map(f => `<div style="font-size: 0.9em; margin-bottom: 2px;"><span style="color: #6b7280; font-weight: 500;">${f.label}:</span> ${f.value}</div>`).join('')
    }
      ...
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="background-color: #f9fafb; padding: 20px; border-radius: 8px;">
          ...
          <div style="display: grid; gap: 15px;">
            ${saleDetails.guests.map((g, index) => `
