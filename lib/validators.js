...
export function getValidationMessage(target, isEn) {
  ...
  if (validity.valueMissing) {
    if (target.type === "checkbox") {
      return dict["validation.checkboxMissing"] || "Please check this box if you want to proceed."
    }
    if (target.tagName?.toLowerCase() === "select") {
      return dict["validation.selectMissing"] || "Please select an item in the list."
    }
    return dict["validation.valueMissing"]
  }

  // Email validation rules
  if (target.type === "email" && (validity.typeMismatch || !target.checkValidity())) {
    if (!val.includes("@")) {
