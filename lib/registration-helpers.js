        ...
        for (const policy of policies) {
          if (age <= policy.max_age) {
            if (policy.use_percentage === false) {
              ...
              total += Number(policy.price !== null ? policy.price : baseForGuest)
...
export function calculateSummaryPrice(
  ...
  const calcTotal = (priceBase) => {
    ...
    if (counts) {
      Object.keys(counts).forEach((key) => {
        ...
        if (policy) {
          if (policy.use_percentage === false) {
            const policyPrice = policy.price != null ? Number(policy.price) : childPriceBase
  ...
  const discountAmount = calculateAdultDiscount(baseAdultTotal, discountPercentage)
