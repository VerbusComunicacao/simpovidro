...
export function getGuestCountsString(saleDetails) {
  ...
  if (saleDetails.guests && Array.isArray(saleDetails.guests)) {
    saleDetails.guests.forEach((guest) => {
      ...
      if (age >= 12) {
        ...
        if (matchingPolicy) {
          const desc = matchingPolicy.description
            ?.replace(/Crianças/g, "Criança(s)")
            .replace(/crianças/g, "criança(s)")
          childrenCounts[desc] = (childrenCounts[desc] || 0) + 1
        } else {
          childrenCounts["Criança(s)"] = (childrenCounts["Criança(s)"] || 0) + 1
        }
      }
    })
  }

  const guestCountsList = [`${adultCount} adulto(s)`]
