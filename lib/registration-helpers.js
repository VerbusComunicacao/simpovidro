...
export function calculateTotalPrice(
  ...
  guests.forEach((guest) => {
    ...
    if (guest.birth_date) {
      age = calculateAge(guest.birth_date, referenceDate)
      isAdult = age >= 12
...
export function calculateMaxInstallments(hotelCheckInDate) {
  ...
  try {
    ...
    // Permite parcelamento enquanto o mês da parcela não ultrapassar o mês da data limite,
    // e a data travada no limite for estritamente posterior à parcela anterior.
    while (keepCalculating) {
      const rawDate = today.add({ months: count })
      if (
        rawDate.year > limitDate.year ||
        (rawDate.year === limitDate.year && rawDate.month > limitDate.month)
      ) {
        keepCalculating = false
        break
      }
      const prevDateRaw = count === 1 ? today : today.add({ months: count - 1 })
      const currentDueDate =
        Temporal.PlainDate.compare(rawDate, limitDate) > 0 ? limitDate : rawDate
      const prevDueDate =
        Temporal.PlainDate.compare(prevDateRaw, limitDate) > 0
          ? limitDate
          : prevDateRaw

      if (Temporal.PlainDate.compare(currentDueDate, prevDueDate) <= 0) {
        keepCalculating = false
        break
      }
      count++
 ...
 */
export function validateRoomCapacity(room, adultCount, childCount, lang) {
  ...
  if (adultCount < minRequired) {
    return {
      isValid: false,
      message: isEn
        ? `This room requires a minimum of ${minRequired} adult guests.`
        : `Este quarto exige no mínimo ${minRequired} hóspedes adultos.`,
    }
  }
  ...
  if (adultCount > maxAdults) {
    return {
      isValid: false,
      message: isEn
        ? `The number of adults (${adultCount}) exceeds the room's maximum capacity (${maxAdults}).`
  ...
  if (childCount > maxChildren) {
    return {
      isValid: false,
      message: isEn
        ? `The number of children (${childCount}) exceeds the room's maximum capacity (${maxChildren}).`
...
export function calculateSalePriceBreakdown(saleDetails) {
  ...
  if (saleDetails.guests && Array.isArray(saleDetails.guests)) {
    saleDetails.guests.forEach((guest) => {
      ...
      const age = calculateAge(guest.birth_date, checkIn)
      if (age >= 12) {
        adultCount++
      }
    })
    ...
    saleDetails.guests.forEach((guest) => {
      ...
      if (age >= 12) {
        ...
        if (matchingPolicy) {
          ...
          const defaultChildLabel = isInternational
            ? "Child(ren)"
            : "Criança(s)"
          childrenCounts[defaultChildLabel] =
            (childrenCounts[defaultChildLabel] || 0) + 1
        }
      }
    })
  }

  const adultLabel = isInternational
    ? `${adultCount} adult(s)`
    : `${adultCount} adulto(s)`
...
/**
 * Safely parses any date string (ISO or DD/MM/YYYY) into a UTC Date object.
 *
...
/**
 * Calculates the age of a guest relative to a reference date (using UTC to prevent timezone issues).
 *
 * @param {string|Date} birthDate - Date of birth.
 * @param {string|Date} referenceDate - Reference date (e.g. check-in date).
...
/**
 * Checks if a guest is an adult for room price/capacity categories (age >= 12).
 *
 * @param {string|Date} birthDate
 * @param {string|Date} referenceDate
 * @returns {boolean}
 */
export function isAdult(birthDate, referenceDate = new Date()) {
  return calculateAge(birthDate, referenceDate) >= 12
}

/**
 * Checks if a guest is a legal adult (age >= 18).
 *
 * @param {string|Date} birthDate
 * @param {string|Date} referenceDate
 * @returns {boolean}
 */
export function isLegalAdult(birthDate, referenceDate = new Date()) {
...
export const ACTIVITY_SECTORS_EN = {
