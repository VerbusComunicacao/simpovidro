...
export function calculateSummaryPrice(
  ...
  const calcTotal = (priceBase) => {
    let total = 0
    const b = Number(priceBase) || 0

    // Adultos
    total += (Number(counts?.adults) || 0) * b
  ...
  const memberBase = Number(room.member_price_per_night || room.price_per_night || 0)
